...
//***********************EVERYTHING COMMENTED OUT BELOW ORIGINAL****************************************
...
function SetUpRotation (centerPos : Vector3, headPos : Vector3)
{
	...
	var yRotation = Quaternion.LookRotation(Vector3(offsetToCenter.x, offsetToCenter.y+clampHeadPositionScreenSpace, offsetToCenter.z));

	var relativeOffset = Vector3.forward * distance + Vector3.down * height;
	cameraTransform.rotation = yRotation * Quaternion.LookRotation(relativeOffset);

	// Calculate the projected center position and top position in world space
	var centerRay = cameraTransform.camera.ViewportPointToRay(Vector3(.5, 0.5, 1));//(.5, 0.5, 1));
	...
	else
	{
		...
		//cameraTransform.rotation *= Quaternion.Euler(-extraLookAngle, 0, 0); /// if its in, camera gets below you when you jump
	}
}
